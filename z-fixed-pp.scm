...
;; This file was copied from Guile 1.8 ice-9/pretty-print.scm,
;; with the following (non-cosmetic) modifications by ttn:
;; - Drop ‘pretty-print-with-keys’.
...
;; Pretty-print OBJ on PORT, or current-output-port if PORT is omitted.
;;
(define (pretty-print obj . opts)
  (generic-write obj #f
		 79
		 ""
		 (let ((port (if (pair? opts)
                                 (car opts)
                                 (current-output-port))))
                   (lambda (s)
                     (display s port)
